...
export default class Contract {
  ...
  async start() {
    ...
    state._contract = new web3.eth.Contract(ABI, contractAddress, {from})
    state.ready = true
  }
  ...
  async newPost(text) {
    return await state._contract.methods.newPost(text).send()
  }

  async newComment(postId, text) {
    return await state._contract.methods.newComment(postId, text).send()
  }
